...
def get_duckdb_connection(database=':memory:'):
    ...
    try:
        ...
        if "Extension" in str(e) and "is an existing extension" in str(e):
            print("Reinstalling httpfs extension...")
            con.execute("FORCE INSTALL httpfs;")
            con.execute("LOAD httpfs;")
            print("httpfs extension reinstalled and loaded successfully.")
        else:
            print(f"Error with httpfs extension: {e}")
            print("Note: S3 operations will not work without the httpfs extension.")
    ...
    print(f"Configuring AWS credentials - Region: {region}, Access Key: {access_key[:6]}..., Bucket: {bucket_name}")
...
def duck_db_parquet_delete_and_insert(
    ...
    if schema:
        ...
        print(f"Executing COPY command to S3: {s3_data_path}")
